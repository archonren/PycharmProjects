# little modification so that it can be run under python 3
# (c) 2014 Reid Johnson
#
# Modified from:
...
def fpgrowth(dataset, min_support, include_support=False, verbose=False):
    ...
    F = []
    support_data = {}
    for k,v in find_frequent_itemsets(dataset, min_support, include_support=True, verbose=verbose):
...
def rules_from_conseq(freq_set, H, support_data, rules, rules_H, min_confidence, verbose=False):
    ...
    m = len(H[0])
    if m == 1:
        Hmp1 = calc_confidence(freq_set, H, support_data, rules, rules_H, min_confidence, verbose)
    if (len(freq_set) > (m+1)):
        Hmp1 = apriori_gen(H, m+1) # generate candidate itemsets
        Hmp1 = calc_confidence(freq_set, Hmp1,  support_data, rules, rules_H, min_confidence, verbose)
        if len(Hmp1) > 1:
            # If there are candidate rules above the minimum confidence 
            # threshold, recurse on the list of these candidate rules.
            rules_from_conseq(freq_set, Hmp1, support_data, rules, rules_H, min_confidence, verbose)

def calc_confidence(freq_set, H, support_data, rules, rules_H, min_confidence, verbose=False):
    ...
    for conseq in H: # iterate over the frequent itemsets
        ...
        if conf >= min_confidence:
            ...
            rules_H[freq_set-conseq] = {"prior":freq_set-conseq,"post":conseq,"sup": str(round(conf, 3)),"con":str(round(support_data[freq_set], 3))}
            if verbose:
                print("" \
                    + "{" \
...
def generate_rules(F, support_data, min_confidence, verbose=False):
    ...
    for i in range(1, len(F)):
        for freq_set in F[i]:
            ...
            if (i > 1):
                rules_from_conseq(freq_set, H1, support_data, rules, rules_H, min_confidence, verbose)
            else:
                calc_confidence(freq_set, H1, support_data, rules, rules_H, min_confidence, verbose)

    return rules,rules_H
